...
Download a database once; all lookups run offline against local .BIN files.
  ...
  python ip2location.py --download DB11 PX7         # download two databases
  ...
  python ip2location.py --json --db DB11 8.8.8.8    # JSON output
...
STALE_DAYS        = 35   # warn when a local database hasn't been refreshed in this many days

# ANSI colour codes — defined early so they're available in every section below.
RESET  = "\033[0m"
BOLD   = "\033[1m"
DIM    = "\033[2m"
CYAN   = "\033[36m"
...
# fields   — canonical field names this database can populate (for documentation)
...
# When no --db is given, auto-mode queries the richest available DB from each group.
...
# IP2Proxy returns short proxy-type codes; expand them to readable strings.
_PROXY_TYPE_LABEL = {
    "VPN": "VPN / Anonymizer",
    "TOR": "Tor Exit Node",
...
def download(names: list, force: bool = False) -> bool:
    ...
    for raw_name in names:
        ...
        if name not in CATALOG:
            print(f"  [ERR]  '{name}' is not in the catalog — run --list to see options",
                  file=sys.stderr)
            all_ok = False
            continue
        ...
        try:
            ...
            with zipfile.ZipFile(io.BytesIO(raw_bytes)) as zf:
                # Find the .BIN file inside the archive (name may be in a subfolder).
                target = next(
...
def list_databases():
    """Print a compact overview of all databases with download status."""
    ...
    print(f"\n{BOLD}Geolocation{RESET}  (IPv6-capable BIN, each level adds fields)")
    for n in ["DB1", "DB3", "DB5", "DB9", "DB11", "ASN"]:
        _line(n)

    print(f"\n{BOLD}Proxy / VPN detection{RESET}  (each level adds fields)")
...
def _open(name: str):
    ...
    if not path.exists():
        print(
            f"[WARN] {name} not downloaded — run: python ip2location.py --download {name}",
            file=sys.stderr,
        )
        _handles[name] = None
        return None
...
# Values that IP2Location / IP2Proxy return when a field is not populated.
...
def _query_geo(name: str, ip: str) -> dict:
    ...
    # Map library record attribute names → our canonical field names.
    result = {}
    for attr, canon in [
...
def lookup(ip: str, db_name: str | None = None) -> dict:
    ...
    Auto mode       (db_name=None):  merge the richest available geo DB + ASN + proxy DB.
    """
    ...
    if db_name:
        ...
        if "_error" not in fields:
            ...
            result["_error"] = fields["_error"]

    else:
        # Geo: use the richest available geo database.
        geo = _best(_GEO_PREF)
        if geo:
            f = _query_geo(geo, ip)
            if "_error" not in f:
...
# Human-readable labels for known field names, in the preferred display order.
_FIELD_ORDER = [
    ("country_code",  "Country"),         # country_name printed on the same line
    ("region",        "Region"),
    ("city",          "City"),
    ("latitude",      "Lat / Lon"),       # longitude printed on the same line
    ("zip",           "ZIP"),
    ("timezone",      "Timezone"),
    ("isp",           "ISP"),
    ("domain",        "Domain"),
...
def print_result(data: dict):
    ...
    src_tag = f"  {DIM}[{', '.join(sources)}]{RESET}" if sources else ""
    ...
    # Lat/Lon: two fields on one line.
    lat = data.get("latitude")
    lon = data.get("longitude")
    if lat and lon:
        _row("Lat / Lon", f"{lat}, {lon}")

    # All other fields in display order (skipping the ones already handled above).
    skip = {"country_code", "country_name", "latitude", "longitude"}
...
def print_table(results: list):
    ...
    for data in results:
        ...
        for i, ((_, w), val) in enumerate(zip(cols, vals)):
            ...
            if has_proxy and i == len(cols) - 1:   # proxy column: colour by result
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python ip2location.py --json --db DB11 8.8.8.8\n"
        ),
    )
    ...
    parser.add_argument("--update",   nargs="*",  metavar="DB",
                        help="Re-download databases. Omit names to update all downloaded ones.")
    parser.add_argument("--force",    action="store_true",
                        help="With --download/--update: skip the freshness check.")
    parser.add_argument("--db",       metavar="DB",
                        help="Query a specific database instead of auto-detecting the best available.")
    ...
    for name in CATALOG:
        if _downloaded(name):
            ...
            if age > STALE_DAYS:
                print(
                    f"[WARN] {name} is {age:.0f} days old"
    ...
    if args.json:
        # Strip internal _keys before serialising.
        def _pub(d):
